...
6. Ejecuta `npm test`
7. Genera `dist\CBSPrintService_1.9.0_Setup.exe`

## Instalación silenciosa (SCCM / GPO)

```batch
CBSPrintService_1.9.0_Setup.exe /VERYSILENT /SUPPRESSMSGBOXES /LOG="C:\cbs_install.log"
```
...
3. Comando de instalación:
   ```
CBSPrintService_1.9.0_Setup.exe /VERYSILENT /SUPPRESSMSGBOXES
   ```
...
3. Agregar script:
   ```batch
   \\domain\NETLOGON\CBS\CBSPrintService_1.9.0_Setup.exe /VERYSILENT
   ```

### Opción 3: PDQ / Lanzadores
```batch
CBSPrintService_1.9.0_Setup.exe /VERYSILENT /SUPPRESSMSGBOXES /LOG="%TEMP%\cbs_install.log"
```
